      ...
      <div style={{ ...styles.root, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(160deg, #0a1628 0%, #0d2145 100%)", padding: 24 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 20, maxWidth: 460, width: "100%", padding: "44px 40px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 28 }}>
            <img src={LOGO_B64} alt="Cadence Compliance" style={{ height: 34, width: 34, borderRadius: 6, objectFit: "cover" }} />
    ...
    return (
      <div style={{ ...styles.root, display: "flex", flexDirection: "column", height: "100vh", background: "#f8f9fb" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 24px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          ...
          <button onClick={() => setView("landing")} style={{ fontSize: 13, color: "#6b7280", background: "none", border: "1px solid #e5e7eb", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}>
            Return to site
          </button>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "32px 24px", maxWidth: 760, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
          {messages.map((msg, i) => (
            <div key={i} style={{ marginBottom: 20, display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
              {msg.role === "assistant" && (
                <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#0a84ff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginRight: 10, marginTop: 2 }}>
                  <ShieldIcon size={14} color="#fff" />
                </div>
              )}
              <div style={{ maxWidth: "78%", background: msg.role === "user" ? "#0a84ff" : "#fff", color: msg.role === "user" ? "#fff" : "#0f1117", border: msg.role === "assistant" ? "1px solid #e5e7eb" : "none", borderRadius: msg.role === "user" ? "16px 16px 4px 16px" : "4px 16px 16px 16px", padding: "12px 16px", fontSize: 14, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
                {msg.content}
              </div>
            </div>
          ))}
          {chatLoading && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#0a84ff", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <ShieldIcon size={14} color="#fff" />
              </div>
              <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: "4px 16px 16px 16px", padding: "12px 16px" }}>
                <div style={{ display: "flex", gap: 4 }}>
                  {[0, 0.15, 0.3].map((d, i) => (
                    <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#9ca3af", animation: "pulse 1s infinite", animationDelay: `${d}s` }} />
                  ))}
                </div>
              </div>
            </div>
        ...
        <div style={{ background: "#fff", borderTop: "1px solid #e5e7eb", padding: "16px 24px", flexShrink: 0 }}>
          ...
          <p style={{ fontSize: 11, color: "#9ca3af", textAlign: "center", marginTop: 10 }}>This tool provides general compliance information only. For advice specific to your organisation, book a consultation.</p>
        </div>
      ...
      <footer style={styles.footer}>
        <div style={styles.footerInner}>
          <div className="footer-grid" style={styles.footerGrid}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <img src={LOGO_B64} alt="Cadence Compliance" style={{ height: 40, width: 40, borderRadius: 8, objectFit: "cover" }} />
